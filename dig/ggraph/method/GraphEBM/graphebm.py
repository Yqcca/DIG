...
class GraphEBM(Generator):
    ...
    """
    def __init__(self, n_atom, n_atom_type, n_edge_type, hidden, atype, device=None):
        super(GraphEBM, self).__init__()
        if device is None:
            self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        else:
            self.device = device
        self.energy_function = EnergyFunc(n_atom_type, hidden, n_edge_type, atype).to(self.device)
        self.n_atom = n_atom
    ...
    def train_goal_directed(self, loader, lr, wd, max_epochs, c, ld_step, ld_noise, ld_step_size, clamp, alpha, save_interval, save_dir, metric):
        r"""
        ...
        for epoch in range(max_epochs):
            ...
            for _, batch in enumerate(tqdm(loader)):
                ...
                if metric == 'drd2':
                    ...
                    batch.y = ADRD2(batch.smile)
                    pos_y = torch.Tensor(batch.y).to(self.device)
                elif metric == 'median1':
                    AMEDIAN1 = Oracle(name = 'median1')
                    batch.y = AMEDIAN1(batch.smile)
                    pos_y = torch.Tensor(batch.y).to(self.device)
                elif metric == 'median2':
                    AMEDIAN2 = Oracle(name = 'median2')
                    batch.y = AMEDIAN2(batch.smile)
                    pos_y = torch.Tensor(batch.y).to(self.device)
                elif metric == 'qed':
